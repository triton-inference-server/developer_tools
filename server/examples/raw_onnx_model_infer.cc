...
void check_status(OrtStatus* status) {
    if (status != NULL) {
        const char* msg = OrtGetApiBase()->GetApi(ORT_API_VERSION)->GetErrorMessage(status);
        printf("Error: %s\n", msg);
...
int main() {
    ...
    check_status(g_ort->SessionOptionsAppendExecutionProvider_CUDA(session_options, &cuda_options)); // 0 for the default GPU device

    // Load the ONNX model
    const char* model_path = "/work/server/build/install/bin/models/resnet50/1/resnet50-1.2.onnx";
    OrtSession* session;
    ...
    check_status(g_ort->CreateTensorWithDataAsOrtValue(memory_info, input_tensor_values, 
        elem_cnt * sizeof(float), input_node_dims, num_dims, ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT, &input_tensor));
    ...
    check_status(g_ort->Run(session, NULL, input_names, (const OrtValue* const*)&input_tensor, 1, output_names, 1, &output_tensor0));
    check_status(g_ort->Run(session, NULL, input_names, (const OrtValue* const*)&input_tensor, 1, output_names, 1, &output_tensor1));
    check_status(g_ort->Run(session, NULL, input_names, (const OrtValue* const*)&input_tensor, 1, output_names, 1, &output_tensor2));
    }
    ...
    // Calculate the elapsed time in milliseconds
     ...
     std::cout << "Running Avg Elapsed time: " << total_elapsed.count()/i << " ms" << std::endl;
    }

    // Output the elapsed time
    std::cout << "Avg Elapsed time: " << total_elapsed.count()/100 << " ms" << std::endl;
    

    // Retrieve the output data
    //float* output_data;
    //check_status(g_ort->GetTensorMutableData(output_tensor0, (void**)&output_data));

...
#include <triton/developer_tools/triton/input.hpp>
#include <triton/developer_tools/triton/requests.hpp>
#include <triton/developer_tools/triton/responses.hpp>
 ...
 * requests
 *
 * Batch objects are the primary interface point between developer_tools::backend
 * Models and the Triton server itself. By calling the `get_input` and
 * `get_output` methods of a batch, Model implementations can retrieve the input
 * Tensors necessary for prediction and the output Tensors where results can be
 * stored.
 *
 ...
 *
 * It is not recommended that developers of developer_tools backends try to
 * construct Batch objects directly. Instead, you should make use of the
 * developer_tools::backend::triton_api::execute template, which will construct
 * the Batch for you.
 */
struct Batch {
  ...
  Batch(TRITONBACKEND_Request** raw_requests,
    ...
    : requests_{raw_requests, raw_requests + count},
      ...
      responder_{std::make_shared<triton::backend::BackendOutputResponder>(raw_requests,
                                                          count,
                                                          &responses_,
                                                          max_batch_size,
                                                          &triton_mem_manager,
 ...
 private:
  ...
  std::function<void(TRITONBACKEND_Request*,
                     ...
                     time_point const&)>
    report_statistics_;
  triton::backend::BackendInputCollector collector_;
  std::shared_ptr<triton::backend::BackendOutputResponder> responder_;

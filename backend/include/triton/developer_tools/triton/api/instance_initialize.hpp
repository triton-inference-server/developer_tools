...
auto* instance_initialize(TRITONBACKEND_ModelInstance* instance)
{
  ...
  try {
    ...
    auto tools_model = std::make_unique<ModelInstanceState>(*model_state, instance);
    if constexpr (IS_GPU_BUILD) {
      auto& model = tools_model->get_model();
    ...
    set_instance_state<ModelInstanceState>(*instance, std::move(tools_model));
